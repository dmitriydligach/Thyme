...
class T5EncoderOnly(T5Model):
  ...
  def __init__(self, config, num_classes):
    ...
    super(T5EncoderOnly, self).__init__(config)

    self.t5encoder = T5EncoderModel(config)
  ...
  def forward(self, input_ids, attention_mask):
    ...
    output = self.t5encoder(input_ids, attention_mask)[0]
...
def perform_fine_tuning():
  ...
  model.resize_token_embeddings(32128) # t5 vocab_size is 32128
